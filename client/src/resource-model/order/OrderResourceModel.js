...
export default class OrderResourceModel extends AbstractResourceModel {
    ...
    constructor(props) {
        ...
        this.state = {
            resourceName: 'Order',
            dataType: SyncConstant.TYPE_ORDER
        };
    }
     ...
     */
    placeOrder(order) {
        return this.saveToDb([order], true).then(() => {
            return order;
        });
    }

    /**
     * Get date of order from since date
     */
    getConfigSinceDate() {
        let date = new Date();
    ...
    async getOutDateOrders() {
        ...
        let date = this.getConfigSinceDate();
    ...
    async refund(creditmemo) {
        fire('resource_model_order_refund', {
    ...
    getOutOfPermissionOrders(queryService = {}, isSync = false) {
        return this.getResourceOnline().getOutOfPermissionOrders(queryService, isSync);
    }
